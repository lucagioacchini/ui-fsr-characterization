...
# -*- coding: utf-8 -*-
"""
This script defines all the graphic features of the fsrTool GUI. 
...
import PIL.ImageTk
import os
import docs.src.config as config
...
#TAB 2 LOWER SCREEN: GRAPHICS
		...
		self.tab_1_graph = self.notebook_graph.add(config.name_3)
		self.tab_2_graph = self.notebook_graph.add(config.name_1)
		self.tab_3_graph = self.notebook_graph.add(config.name_2)
		self.tab_4_graph = self.notebook_graph.add("Merged")
...
#TAB_1 FUNCTIONS				
	...
	def sub_weight(self):
		...
		resistance=sensor.detect_value(weight)
